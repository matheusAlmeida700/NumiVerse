...
import { Sun, Moon, Earth, Calculator, Circle, Square, Triangle } from 'lucide-react';
    ...
    <div className="w-full h-screen relative overflow-hidden">
      ...
      {planets.map((planet, index) => {
        ...
        return (
          <div
            key={planet.id}
            className={`absolute ${isTheSun ? 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10' : 'left-1/2 top-1/2'}`}
            style={!isTheSun ? { 
              animation: `orbit ${planet.orbitSpeed}s linear infinite${activeId ? ' paused' : ''}`,
              transform: `rotate(${(index-1) * (360 / (planets.length-1))}deg) translateX(${225 + (index-1) * 75}px) rotate(-${(index-1) * (360 / (planets.length-1))}deg)`,
            } : undefined}
          >
            <div 
